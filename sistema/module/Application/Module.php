...
class Module extends AbstractModule
{
    ...
    public function checkAcl(MvcEvent $event)
    {
        ...
        if ($usuarioLogado && !$usuarioLogado->isVisivel()) {
            $usuarioLogado = null;
        }
        ...
        if ($controller != 'usuarios.controller.login' && (!$usuarioLogado || !$acl->hasResource($controller) || $usuarioLogado->getClienteId() != SM::getCliente()->id || !$acl->isAllowed($usuarioLogado->getPerfil(), $controller))) {
            $response = $event->getResponse();
