...
class LoginController extends AbstractActionController
{
    ...
    public function loginAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($form->isValid()) {
                $prms = $form->getData();
                $usuario = $this->usuariosService->login($prms['usuario']['email'], $prms['usuario']['senha']);
            }
            if ($usuario) {
                $this->usuariosSession->setUsuarioLogado($usuario);
                // Registra uma log do login do usuário
                $logger = SM::get('app_logger');
                $logger->info("LOGIN: {$usuario->getId()} ({$usuario->getEmail()}).");
                //
                ...
                $retorno['flashError'] = 'Não foi possível realizar o login.';
            }
